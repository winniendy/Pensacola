...
sub create_comparison {
    ...
    foreach ( sort { $a <=> $b } keys %snp_location ) {
        ...
        if ( !defined $gap_location{$_} ) {
            ...
            foreach my $comparison (@header_list) {
                if ( defined $snp_location{$_}{$comparison} ) {
                    print COMP "$snp_location{$_}{$comparison}\t";
                }
                elsif ( !defined $snp_location{$_}{$comparison} ) {
        ...
        if ( $column !~ /contig/ && $column !~ /read/ ) {
            ...
            foreach my $row (@headers) {
                ...
                if ( $column eq $row ) {
                    ...
                    #         print MAT "$row\t";
                    if ( defined $pairSNPcount{"$column:$row"} ) {
                        print PMAT $pairSNPcount{"$column:$row"}, "\t";
                    }
                    else { print PMAT "0\t"; }

                    if ( defined $coreSNPcount{"$column:$row"} ) {
                        print CMAT $coreSNPcount{"$column:$row"}, "\t";
                    }
                    else { print CMAT "0\t"; }

                    if ( defined $cdsSNPcount{"$column:$row"} ) {
                        print CDSMAT $cdsSNPcount{"$column:$row"}, "\t";
                    }
                    else { print CDSMAT "0\t"; }

                    if ( defined $intSNPcount{"$column:$row"} ) {
                        print IMAT $intSNPcount{"$column:$row"}, "\t";
                    }
                    else { print IMAT "0\t"; }
                }
            }
...
sub SNPcounts {
    ...
    my $query_id  = 0;
    my $ref_id    = 0;
    my ( $ref_pos, $ref_base, $snp, $snp_pos, $buff, $dist, $ref_direction,
        $snp_direction );
        ...
        if ( $ssuffix =~ /vcf/ ) {
            ...
            if ( $_ !~ /^#/ ) {
                    ...
                    && $depth >= $readsmapping_depth_cutoff )
                {
                    ...
                    if ( !defined $coding_location{$ref_pos} ) {
                        $intCount++;
                    }
                    if ( defined $coding_location{$ref_pos} ) { $cdsCount++; }
                }
    ...
    if ( $ref_id =~ /(.+)_(\d+)_(\d+)$/ && !$headers_hash{$ref_id} ) {
        $ref_id = $1;
    }
    ...
    while (<IN>) {
        ...
        if ( !/^\#/ ) {
           ...
           #      print "$ref\t$rpos\t$rbase\t$qbase\t$info1\t$info2\t$tmp\n";
            ...
            #      print "$info[0]\n";
            if ( $info[0] =~ /.+=(\d+)/ ) { $depth = $1; }
            if ( $depth < $readsmapping_depth_cutoff ) {
                $gap_location{$rpos}++;
            }
            if (   $qbase !~ /\./
                && $rbase !~ /\./
                ...
                && $depth >= $readsmapping_depth_cutoff )
            {
                ...
                if ( $qbase =~ /(\D),(\D)/ ) {
                    ...
                    $snp_location{$rpos}{"$reference:$query"} = $qbase;
                    $count++;
                }
